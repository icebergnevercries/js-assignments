...
function removeFirstOccurrences(str, value) {
    return str.slice(0, str.indexOf(value)).concat(str.slice(str.indexOf(value) + value.length));
}
...
function getRectangleString(width, height) {
    let fline = '┌' + '─'.repeat(width - 2) + '┐\n';
    let line = '│' + ' '.repeat(width - 2) + '│\n';
    let lline = '└' + '─'.repeat(width - 2) + '┘\n';
    return fline.concat(line.repeat(height - 2), lline);
}
...
function encodeToRot13(str) {
    const alphabetlow = 'abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz';
    const alphabetup = 'ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZ';
    let solve = '';
    for(let i = 0; i < str.length; i++) {
        if(alphabetlow.indexOf(str[i]) !== -1) {
            solve = solve.concat(alphabetlow[alphabetlow.indexOf(str[i]) + 13]);
        }
        else if(alphabetup.indexOf(str[i]) !== -1) {
            solve = solve.concat(alphabetup[alphabetup.indexOf(str[i]) + 13]);
        }
        else {
            solve = solve.concat(str[i]);
        }
    }
    return solve;
}
