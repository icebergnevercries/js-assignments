...
function getPolynom() {
    ...
    return !params.length ? null : function(value) {
        let result = 0;
        for(let i = 0; i < params.length; i++) {
            result += Math.pow(value, params.length - i - 1) * params[i];
        }
        return result;
    }
}
...
function memoize(func) {
    let mems = new Map();
    return function (value) {
        if (!mems.has(value)) mems.set(value, func(value));
        return mems.get(value);
    }
}
...
function partialUsingArguments(fn) {
    ...
    return function() {
        return fn.apply(null, params.concat(Array.from(arguments)));
    };
}
